    ...
    <div className="space-y-3">
      ...
      <CardHeader className="gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:flex-row md:w-auto">
          <div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground md:top-3" /><Input className="pl-9" placeholder="Buscar operario, placa, orden o estacion" value={search} onChange={(event) => setSearch(event.target.value)} /></div>
          <Button onClick={exportExcel} disabled={exporting} className="w-full bg-emerald-600 hover:bg-emerald-700 sm:w-auto"><Download className="w-4 h-4 mr-1" /> Excel</Button>
        </div>
      </CardHeader>
        ...
        {loading ? <p className="p-8 text-center text-sm text-muted-foreground">Cargando registros...</p> : !filtered.length ? <p className="p-8 text-center text-sm text-muted-foreground">No hay registros para mostrar.</p> : <>
          <div className="space-y-3 p-3 md:hidden">
            {filtered.map((record) => <button type="button" key={record.id} onClick={() => openDetail(record)} className="w-full rounded-lg border bg-card p-3 text-left shadow-sm transition-colors hover:bg-muted/40">
              ...
              <p className="mt-2 text-sm font-medium">{record.profiles?.full_name}</p>
              <p className="text-xs text-muted-foreground">{record.station?.nombre || "Sin estacion"}</p>
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t pt-2 text-xs"><span>{formatNumber(record.galones)} gal</span><span>{formatCurrency(record.valor_total)}</span><span>{formatNumber(record.rendimiento_real)} km/gal</span>{record.alerta_rendimiento && <Badge variant="destructive">alerta</Badge>}</div>
            </button>)}
          </div>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Detalle de tanqueo</DialogTitle><DialogDescription>Consulta los datos, evidencias y comprobante del tanqueo.</DialogDescription></DialogHeader>{selected && <div className="space-y-3 text-sm">
        <div className="grid gap-2 sm:grid-cols-2"><p>Orden: <strong>{selected.fuel_orders?.num}</strong></p><p>Fecha: <strong>{formatDate(selected.fecha)}</strong></p><p>Operario: <strong>{selected.profiles?.full_name}</strong></p><p>Cedula: <strong>{selected.profiles?.cedula || "-"}</strong></p><p>Moto: <strong>{selected.vehicles?.placa}</strong></p><p>Marca: <strong>{selected.vehicles?.marca}</strong></p><p>Estacion: <strong>{selected.station?.nombre || "-"}</strong></p><p>KM: <strong>{formatNumber(selected.kilometraje_actual)}</strong></p><p>Galones: <strong>{formatNumber(selected.galones)}</strong></p><p>Valor: <strong>{formatCurrency(selected.valor_total)}</strong></p><p>Nivel: <strong>{selected.nivel_tanque}</strong></p><p>Rendimiento: <strong>{formatNumber(selected.rendimiento_real)} km/gal</strong></p><p>Alcance: <strong>{formatNumber(selected.alcance_estimado)} km</strong></p></div>
